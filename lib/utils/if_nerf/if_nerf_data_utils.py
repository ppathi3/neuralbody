...
def sample_ray(img, msk, K, R, T, bounds, nrays, split):
    ...
    bound_mask = get_bound_2d_mask(bounds, K, pose, H, W)
    bound_mask = bound_mask[..., np.newaxis]  # Add a new dimension to match msk shape
    msk = msk * bound_mask


    #msk = msk * bound_mask
    '''
    # Within sample_ray function after obtaining bound_mask
    bound_mask = get_bound_2d_mask(bounds, K, pose, H, W)
    bound_mask = bound_mask[..., np.newaxis]  # Add a new dimension to match msk shape
    '''
    # Verify shapes before multiplication
    print("msk shape:", msk.shape)
    print("bound_mask shape:", bound_mask.shape)
    '''

    # Align shapes and perform multiplication
    if msk.shape[:2] != bound_mask.shape[:2]:
      # Resize or reshape bound_mask to match msk's shape if needed
      bound_mask = cv2.resize(bound_mask, (msk.shape[1], msk.shape[0]))
...
def sample_ray_h36m(img, msk, K, R, T, bounds, nrays, split):
    ...
    #msk = msk * bound_mask
    #bound_mask = get_bound_2d_mask(bounds, K, pose, H, W)
    #msk = msk * bound_mask[..., np.newaxis]
    msk = msk * bound_mask
    bound_mask[msk == 100] = 0
    '''
    # Within sample_ray_h36m function after obtaining bound_mask
    bound_mask = get_bound_2d_mask(bounds, K, pose, H, W)
    bound_mask = bound_mask[..., np.newaxis]  # Add a new dimension to match msk shape
    '''
    # Verify shapes before multiplication
