...
class Dataset(data.Dataset):
    ...
    def get_mask(self, i):
        ...
        for nv in range(len(ims)):
            ...
            # Check if the image has three channels before conversion
            if len(msk_cihp.shape) == 3 and msk_cihp.shape[2] == 3:
                # Convert the mask to grayscale if it has three channels
                msk = cv2.cvtColor(msk_cihp, cv2.COLOR_BGR2GRAY)
            else:
                # Use the original image if it's already grayscale
                msk = msk_cihp
